...
// @version       0.31
// @description   컨텍스트 메뉴 + 스크롤 북마크 + 위치 기억 + 페이지 메모(AES-256 자체 암호화) + ContextData AES-256 통합 + 보안 복사 Argon2id/PBKDF2 GitHub Pages 위임
// @match         *://*/*
...
(function () {
    ...
    // ── 보안 URL 복사: GitHub Pages 새 탭 위임 ─────────────────
    // 암호화는 URL Locker 페이지에서 수행 — 이 탭은 즉시 닫힘
    // 새 탭으로 위임하는 이유:
    //   1. 위임 직후 탭이 닫혀 메모리에 pw/url 잔류 없음
    //   2. URL Locker 탭이 암호화 후 클립보드 복사 + 자동 종료
    //   3. 스크립트에 의해 클립보드가 탈취될 가능성 없음
    const LOCKER_BASE = {
        argon2id: 'https://northkoreanuser.github.io/HTML/URL-Locker/',
        pbkdf2:   'https://northkoreanuser.github.io/HTML/URL-Locker/',
        bcrypt:   'https://northkoreanuser.github.io/HTML/URL-Locker/',
        xor:      'https://northkoreanuser.github.io/HTML/URL-Locker/',
    };

    // fmt 숫자: 유저스크립트 format 문자열 → URL Locker fmt 파라미터
    const FMT_NUM = { plain: '1', markdown: '2', tag: '3' };
    const FMT_LABEL = { plain: '일반 (제목 + URL)', markdown: '마크다운', tag: '링크 태그' };

    // 보안 URL 복사 모달: 비밀번호 + 알고리즘 + 힌트 선택 → 새 탭 위임
    function showSecureUrlCopyModal(format = 'plain') {
        const targetUrl = decodeURIComponent(location.href);
        const pageTitle = document.title;
        const fmtNum    = FMT_NUM[format] || '1';

        const { ov, append } = mkModal(360, '#111518');

        let selectedAlgo = 'argon2id';

        // 제목
        const title = el('div');
        title.innerHTML = `🔐 <strong>보안 URL 복사</strong> <span style="font-size:10px;color:#555;font-weight:normal">· ${FMT_LABEL[format]}</span>`;
        css(title, { fontSize:'14px', color:'#4fffb0', marginBottom:'6px', fontFamily:'monospace' });

        // 위임 안내
        const delegateNote = el('div');
        delegateNote.textContent = '암호화는 URL Locker 새 탭에서 처리됩니다. 완료 후 탭이 자동으로 닫힙니다.';
        css(delegateNote, { fontSize:'10px', color:'#4fffb0', background:'#0d1a0f',
        ...
        const ALGO_CFG = {
            argon2id: { label: 'Argon2id ★', desc: 'Argon2id → AES-256-GCM (메모리 하드 · 최강)' },
            pbkdf2:   { label: 'PBKDF2',     desc: 'PBKDF2-SHA256 × 600,000 → AES-256-GCM (호환성)' },
            bcrypt:   { label: 'bcrypt',     desc: 'bcrypt (cost 12) → SHA-256 → AES-256-GCM' },
            xor:      { label: 'XOR ⚠️',    desc: 'XOR 스트림 암호 — 교육용, 보안 강도 낮음' },
        };

        const algoButtons = {};
        Object.entries(ALGO_CFG).forEach(([k, cfg]) => {
            const btn = el('button', { type:'button', textContent: cfg.label });
            css(btn, { flex:'1', padding:'6px 0', borderRadius:'6px', fontSize:'12px',
        ...
        const desc = el('div');
        css(desc, { fontSize:'11px', color:'#666', marginBottom:'14px', lineHeight:'1.6' });
        ...
        css(urlPrev, { fontSize:'10px', color:'#4fffb0', background:'#0d1a0f',
                       border:'1px solid #1a3d2e', borderRadius:'6px',
                       padding:'7px 10px', marginBottom:'14px',
        ...
        const segs = Array.from({ length: 4 }, () => {
            const s = el('div');
            css(s, { flex:'1', height:'3px', background:'#2a2f42', borderRadius:'2px', transition:'background .3s' });
            return s;
        });
        segs.forEach(s => strengthWrap.appendChild(s));
        const strengthLbl = el('div');
        css(strengthLbl, { fontSize:'10px', color:'#888', marginBottom:'10px', letterSpacing:'0.08em' });
        ...
        // 에러
        const errEl = el('div');
        css(errEl, { fontSize:'11px', color:'#ff4f6a', minHeight:'16px', marginBottom:'8px' });
        ...
        const doDelegate = () => {
            ...
            const base = LOCKER_BASE[selectedAlgo] || LOCKER_BASE['argon2id'];
            const delegateUrl = base
                + '?lock=1'
            ...
            ov.remove();
            // 이 탭은 즉시 닫힘 → pw/url 메모리 잔류 없음
        };

        const confirmBtn = mkBtn('🔐 새 탭에서 암호화', '#1a3d2e', doDelegate);
        ...
        append(title, delegateNote, algoWrap, desc, urlPrev, pwWrap, strengthWrap, strengthLbl, hintInput, errEl, btnRow);
        pwInput.focus();
    }
    ...
    moSave.onclick = async () => {
        const stored = loadMemo();
        if (_memoKey && !isMemoEncrypted(stored)) _memoKey = null;
        if (isMemoEncrypted(stored) && !_memoKey) { toast('🔒 잠긴 상태입니다. 잠금 버튼으로 해제 후 수정하세요'); return; }
        if (_memoKey) {
    ...
    function buildCM() {
        ...
        // 보안 URL 복사 — 새 탭 위임, 포맷 3종
        const secSep = el('div'); secSep.style.cssText='border-top:1px solid #eee;margin:3px 0';
